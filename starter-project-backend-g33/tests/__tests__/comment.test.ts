...
describe('Testing comment controllers', () => {
    ...
    it('gets all comments of an article', async () => {
        const response = await request.get('/api/v1/comments/').send({ articleID: articleID })
    ...
    it('gets a comment by id', async () => {
        const response = await request.get(`/api/v1/comments/${commentID}`).send({ articleID: articleID })
        expect(response.status).toBe(200)
        expect(response.body.commentContent).toBe('Hail for free speech!')
    })
    it('cannot get a comment by an invalid id', async () => {
        const response = await request.get('/api/v1/comments/123').send({ articleID: articleID })
        expect(response.status).toBe(404)
    })

    it('creates a comment', async () => {
        const response = await request.post('/api/v1/comments/').send({ commentContent: 'another comment!', commentOwner: 'newCommenter', articleID: articleID })
        expect(response.status).toBe(201)
        expect(response.body.commentContent).toBe('another comment!')
    })
    it('cannot create comment without article id', async () => {
        const response = await request.post('/api/v1/comments/').send({ commentContent: 'another comment!', commentOwner: 'newCommenter' })
    ...
    it('cannot create comment without comment content', async () => {
        const response = await request.post('/api/v1/comments/').send({ commentOwner: 'newCommenter', articleID: articleID })
        expect(response.status).toBe(202)
        expect(response.body.message).toBe('Could not create the comment')
    })
    it('cannot create comment without comment owner', async () => {
        const response = await request.post('/api/v1/comments/').send({ commentContent: 'another comment!', articleID: articleID })
        expect(response.status).toBe(202)
        expect(response.body.message).toBe('Could not create the comment')
    })
    ...
    it('updates a comment', async () => {  
        const response = await request.put(`/api/v1/comments/${commentID}`).send({ commentContent: 'updated comment!', commentOwner: 'newCommenter', articleID: articleID })
        expect(response.status).toBe(200)
        expect(response.body.commentContent).toBe('updated comment!')
    })
    it('cannot update a comment without comment content', async () => {
        const response = await request.put(`/api/v1/comments/${commentID}`).send({ commentOwner: 'newCommenter', articleID: articleID })
        expect(response.status).toBe(400)
    ...
    it('cannot update a comment with the wrong commentID', async () => {
        const response = await request.put(`/api/v1/comments/123`).send({ commentContent: 'updated comment!', commentOwner: 'newCommenter', articleID: articleID })
        expect(response.status).toBe(404)
        expect(response.body.message).toBe('No comment with this ID')
    })

    it('deletes a comment', async () => {
        const response = await request.delete(`/api/v1/comments/${commentID}`).send({ articleID: articleID })
        expect(response.status).toBe(200)
        
    })
    it('cannot delete a comment with an invalid commentID', async () => {
        const response = await request.delete(`/api/v1/comments/123`).send({ articleID: articleID })
        expect(response.status).toBe(404)
        expect(response.body.message).toBe('Could not delete comment')
    })
    it('cannot delete a comment with no commentID', async () => {
        const response = await request.delete(`/api/v1/comments/`).send({ articleID: articleID })
        expect(response.status).toBe(400)
        expect(response.body.message).toBe('Could not delete comment')
    })
