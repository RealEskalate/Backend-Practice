...
describe("GET /users/:userID/posts/:postID/rates/:rateID", () => {
  ...
  it("Fail to get the specified rating", async () => {
    const response = await request
      .get("/users/1/posts/2/rates/62e30c0df63e1f36fc9")
      .send();
    expect(response.status).toBe(400);
  })
});
...
describe("PUT /users/:userID/posts/:postID/rates/:rateID", () => {
  ...
  it("Unable to update nonexistant rate", async () => {
    const response = await request
      .put("/users/1/posts/2/rates/62e30c063e1f36fc9802c2")
      .send({
        rate: 5,
      });
    expect(response.status).toBe(400);
...
describe("DELETE /users/:userID/posts/:postID/rates/:rateID", () => {
  ...
  it("Unable to delete nonexistant rate", async () => {
    const response = await request
      .delete("/users/1/posts/2/rates/62e30c063e1f36fc9802c2")
      .send();
    expect(response.status).toBe(400);
  })
});
